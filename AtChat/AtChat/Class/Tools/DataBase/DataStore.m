...
- (void)createTableWithName:(NSString *)tableName {
    if ([DataStore checkTableName:tableName] == NO) {
        return;
    }
...
- (id)getObjectById:(NSString *)objectId fromTable:(NSString *)tableName {
    DataStoreItem * item = [self getYTKKeyValueItemById:objectId fromTable:tableName];
    if (item) {
...
- (DataStoreItem *)getYTKKeyValueItemById:(NSString *)objectId fromTable:(NSString *)tableName {
    ...
    if (json) {
        ...
        DataStoreItem * item = [[DataStoreItem alloc] init];
...
- (NSArray *)getAllItemsFromTable:(NSString *)tableName {
    ...
    NSError * error;
    for (DataStoreItem * item in result) {
        error = nil;
...
- (void)putArray:(NSArray*)array intoTable:(NSString *)tableName {
    ...
    for (NSDictionary *object in array) {
        ...
        __block BOOL result;
        [_dbQueue inDatabase:^(FMDatabase *db) {
            result = [db executeUpdate:sql, [object objectForKey:@"id"], jsonString, createdTime];
        }];
        if (!result) {
            debugLog(@"ERROR, failed to insert/replace into table: %@", tableName);
        }
...
- (NSArray *)getItemsfromTable:(NSString *)tableName page:(NSUInteger)page rows:(NSUInteger)rows{
    ...
    NSString * sql = [NSString stringWithFormat:QUERY_ITEMS_SQL, tableName];
    __block NSMutableArray * result = [NSMutableArray array];
    [_dbQueue inDatabase:^(FMDatabase *db) {
        FMResultSet * rs = [db executeQuery:sql,rows,(page-1)*rows];
        while ([rs next]) {
            DataStoreItem * item = [[DataStoreItem alloc] init];
            item.itemId = [rs stringForColumn:@"id"];
            item.itemObject = [rs stringForColumn:@"json"];
